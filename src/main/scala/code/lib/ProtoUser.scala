...
trait MetaMegaProtoUser[ModelType <: MegaProtoUser[ModelType]] extends MongoMetaRecord[ModelType] {
  ...
  */
  ...
  private object curUser extends RequestVar[Box[ModelType]](currentUserId.flatMap(id => this.meta.find(id)))  with CleanRequestVarOnSessionTransition
  ...
  */
  ...
  def validateUser(id: String): NodeSeq = this.meta.find(id) match {
  ...
  def login = {
    if (S.post_?) {
      S.param("username").
      flatMap(username => this.meta.find(("email" -> username))) match {
        case Full(user) if user.validated.value &&
  ...
  def sendPasswordReset(email: String) {
    this.meta.find(("email") -> email) match {
      case Full(user) if user.validated.value =>
        user.save
